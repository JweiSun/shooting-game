...
/*
	private void drawImage(Canvas canvas, int x, int y,
		...
		Bitmap tmpImage = Bitmap.createBitmap(w, h, Config.ARGB_8888);
		...
		c.drawBitmap(bsrc, rect_src, rect_dst, null);
		
		boolean isMirror = t_currentTransformation >= TRANS_MIRROR;
		int degrees = (t_currentTransformation % 4) * 90;

		matrix.reset();
		if(isMirror)
			matrix.preScale(-1, 1, dRefX, dRefY);
		matrix.postRotate(degrees, dRefX, dRefY);
		matrix.postTranslate(x, y);
		
		canvas.drawBitmap(tmpImage, matrix, null);
	}
*/
	/**
